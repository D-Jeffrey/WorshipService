...
$sql = "SELECT memberID,mbrStatus,mbrType,mbrEmail1,mbrEmail2,concat(mbrFirstName,' ',mbrLastName) as mbrName FROM members WHERE mbrStatus='A' AND concat(mbrFirstName,' ',mbrLastName) LIKE '%$value%' ORDER BY mbrName";
...
if(mysqli_num_rows($res)>0) {
	while($row = mysqli_fetch_assoc($res)) {
		...
		echo "<li id=\"".$row["memberID"].";".$row["mbrType"].";".$row["mbrEmail1"].";".$row["mbrEmail2"]."\">$match</li>";
	}
}
echo "</ul>";
