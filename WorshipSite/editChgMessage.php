...
$trail->add('Edit Change Request Message', $_SERVER['REQUEST_URI'], 1);
...
// Save values
if(isset($_POST["save"])) {
	$sql = "UPDATE siteconfig SET chgReqMessage='".$_POST["chgReqMessage"]."'";
	$resCfg = $db->query($sql);
...
<head>
<title><?php echo $siteTitle; ?> - Edit Change Request Message</title>
...
$hlpID = 21;
$title = "Edit Change Request Message";
include("header.php");

echo "<h3>See Insert -> Template for template items</H3>";
echo "<form align='center' style='width:100%;margin-top:0px;border-bottom:2px ridge;border-top:2px ridge;' name=\"frm\" action=\"editChgMessage.php\" method=\"post\">\n";
echo "<textarea name='chgReqMessage' id='chgReqMessage' rows='15' style='width: 100%'>".$dbCfg["chgReqMessage"]."</textarea>\n";
