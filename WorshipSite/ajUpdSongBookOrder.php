...
if (isset($_POST['sortableListsSubmitted'])) {
	...
	foreach($orderArray as $item) {
		$sql = "UPDATE songbooksongs set songOrder=".$item['order']."*10 WHERE bookID=$bookID AND songID=".$item['element'];
		$db->query($sql);
	}
	echo "<script>parent.window.dspSong($bookID);parent.window.hs.close();</script>";
	exit;
}

$sql = "SELECT a.songID, b.songName, songOrder from songbooksongs a INNER JOIN songs b ON a.songID=b.songID WHERE bookID=$bookID order by songOrder";
$recordSet = $db->query($sql);
$listArray = array();
while ($record = mysqli_fetch_assoc($recordSet)) {
	$listArray[] = sprintf($listItemFormat,$record['songID'],$record['songName']);
}
...
$sortableLists->printForm($_SERVER['PHP_SELF']."?bid=$bookID", 'POST', 'Save', 'button');
