...
echo "</table>\n";
echo "<form style='margin-top:0px;border-bottom:2px ridge;border-top:2px ridge;' name=\"frm\" action=\"dspSvcSong.php\" method=\"post\">\n";
echo "<table width='100%'><tr><td>\n";

$aSText = explode("<br />",nl2br(str_replace("\r\n","<br />",$oldTxt)));
$noChordsExist = strpos($oldTxt,"[")===FALSE;
if(!$noChordsExist) {
    echo "<input type='button' onClick='setChord();' value='Show/Hide Chords'$dis>\n";
    echo "&nbsp;&nbsp;&nbsp;Semi-tone adjust:&nbsp;<select id=\"selKey\" name=\"semitone\" onChange=\"changeKey();\"$dis>\n";
    for($i=11;$i>0;$i--) {
...
echo "</td><td align='right'><input name=\"back\" type=\"button\" value=\"Back\" onClick=\"document.location='editService.php?id=$serviceID';\" class=\"button\"></td></tr></table></form>\n";
...
for($i=0;$i<count($aSText);$i++) {
	...
	if(substr($aSText[$i],0,1)=="~") { /* Song Part */
		...
		$cn++;
		$aNewSText[] = "	<tr id='orderDetails'><td>".$songTemp."</td></tr>\n";
		$tn++;
	}
}
...
echo "<form><table width='100%'><tr>";
if(!$noChordsExist) {
    echo "<td><input type='button' onClick='setChord();' value='Show/Hide Chords'$dis></td>";
}
echo "<td align='right'><input name=\"back\" type=\"button\" value=\"Back\" onClick=\"document.location='editService.php?id=$serviceID';\" class=\"button\"></td></tr></table></form>\n";
